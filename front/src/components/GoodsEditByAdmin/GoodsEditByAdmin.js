...
import { changeQuantityGoodAC, deleteFromApplicationAC } from '../../utils/redux/actionCreators';
...
export default function GoodsEditByAdmin({el}) {
    ...
    state.adminReducer.applications.find((application) => application.regnumber == id).goods.find((good) => good.title === el.title))
    ...
    const handlerIncreaseGoods = ()=>{
        ...
        dispatch(changeQuantityGoodAC({value:itemInCart.value, regnumber:id, goodTitle:el.title}))
    ...
    const handlerDecreaseGoods = ()=>{
        if (+itemInCart.value !== 1){
            itemInCart.value = itemInCart.value - 1
            dispatch(changeQuantityGoodAC({value:itemInCart.value, regnumber:id, goodTitle:el.title}))
        }else {console.log(itemInCart.value);
        dispatch(deleteFromApplicationAC({regnumber: id, goodTitle:el.title}))}
    }
    ...
    if (+goodValue.current.value === 0) {
      ...
      dispatch(
        dispatch(changeQuantityGoodAC({value:+goodValue.current.value, regnumber:id, goodTitle:el.title}))
      );
    }
    }
